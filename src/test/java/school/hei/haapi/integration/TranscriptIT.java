...
class TranscriptIT {
    ...
    public static Transcript someCreatableTranscript(String studentId) {
        Faker faker = new Faker();
        return new Transcript().id("transcript_" + randomUUID())
                .studentId(studentId)
                .semester(Transcript.SemesterEnum.valueOf("S" + faker.number().numberBetween(1, 6)))
                .academicYear(faker.number().numberBetween(2021, 2024))
                .isDefinitive(false);
    }
    ...
    @Transactional
    @Rollback
    void manager_write_new_transcript_ok() throws ApiException {
        ApiClient manager1Client = anApiClient(MANAGER1_TOKEN);
        TranscriptApi api = new TranscriptApi(manager1Client);

        // create transcript
        List<Transcript> actual = api.crudStudentTranscripts(STUDENT1_ID,
                List.of(someCreatableTranscript(STUDENT1_ID)));
        // get the transcript ordered by creationDatetime desc
        List<Transcript> expected = api.getStudentTranscripts(STUDENT1_ID, 1, 5);

        assertEquals(1, actual.size());
        // check if the just created transcript have creationDatetime
        assertNotNull(expected.get(0).getCreationDatetime());
    }
